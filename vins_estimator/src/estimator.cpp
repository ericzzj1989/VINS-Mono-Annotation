...
/**
 * @brief           处理图像特征数据
 * @Description     addFeatureCheckParallax()添加特征点到feature中，计算点跟踪的次数和视差，判断是否为关键帧               
 *                  判断并进行外参标定
...
{
    TicToc t_sfm;
    // 1. 通过计算线加速度的标准差，判断IMU是否有充分运动激励（是否可观），以进行初始化
    {
        ...
        Vector3d aver_g;
        aver_g = sum_g * 1.0 / ((int)all_image_frame.size() - 1); // 均值，总帧数减1，因为all_image_frame第一帧不算
    ...
    for (auto &it_per_id : f_manager.feature) // list<FeaturePerId> feature 滑动窗口内所有特征点
    {
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        if((frame_it->first) == Headers[i].stamp.toSec())
        {
            ...
            frame_it->second.R = Q[i].toRotationMatrix() * RIC[0].transpose(); // R_c0_ck * (R_b_c)-1 = R_c0_ck * R_c_b = R_c0_ck * R_ck_bk = R_c0_bk = frame_it->second.R
            frame_it->second.T = T[i]; // p_c0_ck
            i++;
            continue;
        }
        if((frame_it->first) > Headers[i].stamp.toSec())
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (corres.size() > 20)
        {
            ...
            // 3. 判断是否满足初始化条件：视差>30和内点数满足要求；同时返回窗口最后一帧（当前帧）到第l帧（参考帧）的Rt
            if(average_parallax * 460 > 30 && m_estimator.solveRelativeRT(corres, relative_R, relative_T)) // 这里的R,T是最后一帧相对于l帧坐标系的位姿估计
            {
                l = i;
